...
public:
  ...
  void OnMouse(const int &event, const int &x, const int &y, const int &flags) {
    ...
    // Debug: print first mouse event
    static bool first_event = true;
    if (first_event) {
      std::cout << "[DEBUG] Mouse event detected! Region display enabled." << std::endl;
      first_event = false;
    }
    ...
    if (event == cv::EVENT_MOUSEMOVE) {
      ...
      std::cout << "[DEBUG] Mouse clicked at (" << x << ", " << y << ")" << std::endl;
      if (selected_) {
        if (x >= static_cast<int>(point_.x - n_) &&
            ...
            y <= static_cast<int>(point_.y + n_)) {
          selected_ = false;
          std::cout << "[DEBUG] Region deselected." << std::endl;
        }
      } else {
        selected_ = true;
        std::cout << "[DEBUG] Region selected." << std::endl;
      }
      point_.x = x;
      point_.y = y;
    }
            ...
            if (left_hip.confidence > 0.6f) {
              ...
              if (px >= 0 && px < depth_data.cols && py >= 0 && py < depth_data.rows) {
                ...
                if (Z > 0 && Z < 10000) {
                  ...
                  // Display 3D coordinates as text overlay
                  std::ostringstream coord_text;
                  coord_text << "Left Hip (KP11): [" << std::fixed << std::setprecision(1)
                            << X << ", " << Y << ", " << Z_val << "] mm";

                  // Position text below the keypoint with background
                  int text_x = px - 100;
                  int text_y = py + 30;

                  // Keep text within image bounds
                  if (text_x < 0) text_x = 5;
                  if (text_y > display.rows - 5) text_y = py - 10;
