...
//
// YOLO Pose Detector Implementation - GPU Accelerated Version
// Modified: 2025-11-20 - Added CUDA GPU support
...
YOLOPoseDetector::YOLOPoseDetector(
    ...
    // ============================================
    // 🚀 GPU ACCELERATION - CUDA Provider
    // ============================================
    // Try to enable CUDA GPU acceleration
    // If CUDA is not available, will automatically fall back to CPU

    std::cout << "Attempting to enable CUDA GPU acceleration..." << std::endl;

    try {
        OrtCUDAProviderOptions cuda_options;

        // GPU Device Configuration
        cuda_options.device_id = 0;  // Use first GPU (change to 1, 2... for multi-GPU systems)

        // Memory Management
        cuda_options.arena_extend_strategy = 0;  // Arena growth strategy
        cuda_options.gpu_mem_limit = 2ULL * 1024 * 1024 * 1024;  // 2GB VRAM limit (adjust based on your GPU)

        // Performance Optimization
        cuda_options.cudnn_conv_algo_search = OrtCudnnConvAlgoSearchExhaustive;  // Find best conv algorithm
        cuda_options.do_copy_in_default_stream = 1;  // Use default stream for data transfer

        // Optional: Enable mixed precision (FP16) for even faster inference
        // Requires GPU with Tensor Cores (RTX series, V100, A100, etc.)
        // Uncomment the line below to enable:
        // cuda_options.cudnn_conv_use_max_workspace = 1;

        session_options_.AppendExecutionProvider_CUDA(cuda_options);
        std::cout << "✓ CUDA GPU acceleration enabled successfully!" << std::endl;
        std::cout << "  Device ID: " << cuda_options.device_id << std::endl;
        std::cout << "  VRAM Limit: " << (cuda_options.gpu_mem_limit / 1024 / 1024) << " MB" << std::endl;

    } catch (const Ort::Exception& e) {
        std::cerr << "⚠ Failed to enable CUDA GPU acceleration: " << e.what() << std::endl;
        std::cerr << "⚠ Falling back to CPU inference" << std::endl;
        std::cerr << "  Possible reasons:" << std::endl;
        std::cerr << "  1. CUDA/cuDNN not installed" << std::endl;
        std::cerr << "  2. ONNX Runtime GPU version not installed" << std::endl;
        std::cerr << "  3. GPU driver issue" << std::endl;
        std::cerr << "  Run 'nvidia-smi' to verify GPU is accessible" << std::endl;
    }

    // CPU Optimization (used as fallback or alongside GPU)
