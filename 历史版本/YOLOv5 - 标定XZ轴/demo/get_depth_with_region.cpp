...
int main(int argc, char **argv) {
  ...
  while (true) {
    if (!depth_queue.empty() && !disparity_queue.empty()) {
      ...
      cv::Mat disparity_display, depth_data;
      {
        std::unique_lock<std::mutex> lock(mutex_disparity);
        disparity_display = disparity_queue.front().clone();
        clear(disparity_queue);
      }
      {
        std::unique_lock<std::mutex> lock(mutex_depth);
        depth_data = depth_queue.front().clone();
        clear(depth_queue);
      }

      // Draw rectangle on the disparity image for display
      depth_region.DrawRect(disparity_display);
      cv::imshow("depth", disparity_display);

      cv::setMouseCallback("depth", OnDepthMouseCallback, &depth_region);

      // Show detailed depth values in region window
