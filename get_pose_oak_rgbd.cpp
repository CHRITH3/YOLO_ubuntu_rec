...
//
// Human Pose Detection using OAK CAM_A RGBD input
// Replaces the original INDEMIND input layer with DepthAI/OAK RGBD frames.
// RGB: CAM_A 640x400 BGR. Depth: CV_16UC1 millimeters aligned to CAM_A.
//
...
int main(int argc, char **argv) {
  ...
  std::cout << "\n=== YOLO Pose Detection with OAK CAM_A RGBD ===\n" << std::endl;
  std::cout << "Using: OAK-FFC-4P RVC2, CAM_A AR0234 RGB + CAM_B/C OV9282 stereo" << std::endl;

  OakRgbdConfig oak_cfg;
  oak_cfg.rgb_width = 640;
  oak_cfg.rgb_height = 400;
  oak_cfg.mono_width = 640;
  oak_cfg.mono_height = 400;
  oak_cfg.fps = 50.0f;
  oak_cfg.pair_threshold_ms = 5.0;
  oak_cfg.exposure_us = 5000;
  ...
  cv_in_left = oak_capture.GetCameraMatrix();
  cv_in_left_inv = oak_capture.GetCameraMatrixInv();
  if (cv_in_left.empty() || cv_in_left_inv.empty()) {
    std::cerr << "ERROR: Failed to read CAM_A intrinsics from DepthAI calibration." << std::endl;
    oak_capture.Stop();
    return -1;
  }

  std::cout << "\nOAK CAM_A Intrinsics @ 640x400:" << std::endl;
  std::cout << "  fx: " << cv_in_left.at<double>(0, 0)
            << ", fy: " << cv_in_left.at<double>(1, 1) << std::endl;
  std::cout << "  cx: " << cv_in_left.at<double>(0, 2)
            << ", cy: " << cv_in_left.at<double>(1, 2) << std::endl;
  ...
  while (true) {
    ...
    if (!left_image.empty()) {
      ...
      cv::putText(display, "OAK CAM_A RGBD", cv::Point(10, 25),
    ...
    if (!depth_data.empty()) {
      ...
      if (std::chrono::duration_cast<std::chrono::seconds>(now - last_debug).count() >= 2) {
        std::cout << "[DEBUG] No depth data available for region display. Depth count: "
                  << oak_capture.DepthCount() << std::endl;
        last_debug = now;
      }
    }
  ...
  std::cout << "Total images captured: " << oak_capture.ImageCount() << std::endl;
  std::cout << "Total depth maps: " << oak_capture.DepthCount() << std::endl;
  std::cout << "Total pose detections: " << pose_count << std::endl;
  std::cout << "Dropped image frames: " << oak_capture.DroppedRgbCount() << std::endl;
  std::cout << "Dropped depth frames: " << oak_capture.DroppedDepthCount() << std::endl;

  if (total_time > 0) {
    std::cout << "\nAverage rates:" << std::endl;
    std::cout << "  Image capture: " << (oak_capture.ImageCount() / total_time) << " FPS" << std::endl;
    std::cout << "  Depth: " << (oak_capture.DepthCount() / total_time) << " FPS" << std::endl;
...
/*
================================================================================
                USAGE INSTRUCTIONS - OAK CAM_A RGBD CAMERA POSE
================================================================================

PROGRAM NAME: get_pose_oak_rgbd
VERSION: 2.0 (YOLO Pose - OAK CAM_A RGBD with 3D Coordinate Inspection)
...
MOUSE INTERACTION:
  On "YOLO Pose - OAK CAM_A RGBD" window:
...
WINDOWS DISPLAYED:
  1. "YOLO Pose - OAK CAM_A RGBD": Main pose detection display
...
OUTPUT INFORMATION:
  On screen:
    ...
    - Camera indicator (OAK CAM_A RGBD)
